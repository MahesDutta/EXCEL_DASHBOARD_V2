...
function ChartCard({ title, subtitle, children, wide = false }) {
  return <section className={`chart-card${wide ? " chart-wide" : ""}`} style={{ overflow: "hidden" }}><div className="chart-head"><h3>{title}</h3>{subtitle && <p>{subtitle}</p>}</div><div className="chart-area">{children}</div></section>;
}
...
function VisualControls({ rows, columns, state, setState, showMetric = true, showDimension = false, showFilter = true, label = "Chart filters" }) {
  ...
  return <div style={{ display: "flex", flexWrap: "wrap", gap: 8, margin: "0 0 12px" }} aria-label={label}>
    {showMetric && metrics.length > 0 && <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "var(--muted)" }}><span>Metric</span><select value={state.metric || ""} onChange={(event) => setState((current) => ({ ...current, metric: event.target.value }))} style={{ maxWidth: 150, minHeight: 32, border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)", color: "var(--text)", padding: "5px 8px" }}>{metrics.map((column) => <option key={column.header} value={column.header}>{column.header}</option>)}</select></label>}
    {showDimension && dimensions.length > 0 && <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "var(--muted)" }}><span>Group by</span><select value={state.dimension || ""} onChange={(event) => setState((current) => ({ ...current, dimension: event.target.value }))} style={{ maxWidth: 150, minHeight: 32, border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)", color: "var(--text)", padding: "5px 8px" }}>{dimensions.map((column) => <option key={column.header} value={column.header}>{column.header}</option>)}</select></label>}
    {showFilter && filterColumns.length > 0 && <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "var(--muted)" }}><span>Filter</span><select value={state.filterColumn || ""} onChange={(event) => setState((current) => ({ ...current, filterColumn: event.target.value, filterValue: "" }))} style={{ maxWidth: 145, minHeight: 32, border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)", color: "var(--text)", padding: "5px 8px" }}><option value="">All data</option>{filterColumns.map((column) => <option key={column.header} value={column.header}>{column.header}</option>)}</select></label>}
    {showFilter && filterColumn && <select aria-label="Filter value" value={state.filterValue || ""} onChange={(event) => setState((current) => ({ ...current, filterValue: event.target.value }))} style={{ maxWidth: 160, minHeight: 32, border: "1px solid var(--border)", borderRadius: 8, background: "var(--surface)", color: "var(--text)", padding: "5px 8px" }}><option value="">All {filterColumn.header}</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>}
  </div>;
}

function controlState(metric, dimension = "") {
  return { metric: metric?.header || "", dimension, filterColumn: "", filterValue: "" };
}
...
function MapCard({ rows, title = "Location map", metric, columns, state, setState }) {
  const [geoJson, setGeoJson] = useState(null);
  const [mapError, setMapError] = useState("");
  const [hovered, setHovered] = useState(null);
  const [hoverPoint, setHoverPoint] = useState({ x: 16, y: 16 });
  ...
  const mapRows = useMemo(() => applyVisualFilter(rows, state), [rows, state]);
  const mapMetric = useMemo(() => (columns || []).find((column) => column.header === state.metric) || metric || null, [columns, state.metric, metric]);
  ...
  // IMPORTANT: keep this hook above every conditional return so the hook order
  // never changes while the GeoJSON map is loading.
  ...
  return <ChartCard wide title={title} subtitle={`Detected ${matchedStates.length} place${matchedStates.length === 1 ? "" : "s"} in the filtered data. Hover a state to see its filtered records${mapMetric ? ` and ${mapMetric.header} total` : ""}.`}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 190px", gap: 18, height: 430, minHeight: 360, maxHeight: 430, overflow: "hidden", alignItems: "stretch" }}>
      <div style={{ position: "relative", minWidth: 0, minHeight: 0, height: "100%", overflow: "hidden" }}>
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label="India state map" style={{ width: "100%", height: "100%", maxHeight: "100%", display: "block", overflow: "hidden" }}>
          {features.map((feature, index) => {
            const name = getGeoName(feature);
            const count = stateCounts.get(name) || 0;
            const active = count > 0;
            return <path key={`${name}-${index}`} d={featurePath(feature, bounds, width, height)} fill={colorForState(name, count)} stroke="var(--border-strong)" strokeWidth={active ? 1.15 : 0.75} vectorEffect="non-scaling-stroke" style={{ cursor: active ? "pointer" : "default", transition: "fill .18s ease, opacity .18s ease", pointerEvents: "all" }} opacity={hovered && hovered !== name ? 0.55 : 1} onPointerEnter={(event) => { setHovered(name); setHoverPoint({ x: Math.min(event.nativeEvent.offsetX + 14, 430), y: Math.max(event.nativeEvent.offsetY - 46, 12) }); }} onPointerMove={(event) => { setHovered(name); setHoverPoint({ x: Math.min(event.nativeEvent.offsetX + 14, 430), y: Math.max(event.nativeEvent.offsetY - 46, 12) }); }} onPointerLeave={() => setHovered(null)} onClick={() => { if (active) setHovered(name); }} onTouchStart={() => { if (active) setHovered(name); }} />;
          })}
        </svg>
        {hovered && <div style={{ position: "absolute", left: Math.max(8, Math.min(hoverPoint.x, 420)), top: Math.max(8, Math.min(hoverPoint.y, 335)), zIndex: 5, minWidth: 175, maxWidth: 235, padding: "9px 12px", borderRadius: 10, background: "var(--surface)", border: "1px solid var(--border)", boxShadow: "var(--shadow-sm)", fontSize: 12, pointerEvents: "none" }}><strong>{hovered}</strong><span style={{ display: "block", color: "var(--muted)", marginTop: 3 }}>{formatFullNumber(stateCounts.get(hovered) || 0)} matching record{(stateCounts.get(hovered) || 0) === 1 ? "" : "s"}{mapMetric && <><br />{mapMetric.header}: {formatFullNumber(stateMetricTotals.get(hovered) || 0)}</>}</span></div>}
      </div>
...
function App() {
  ...
  const [kpiControls, setKpiControls] = useState({ metric: "", filterColumn: "", filterValue: "" });
  const [trendControls, setTrendControls] = useState({ metric: "", dimension: "", filterColumn: "", filterValue: "" });
  const [monthlyControls, setMonthlyControls] = useState({ metric: "", dimension: "", filterColumn: "", filterValue: "" });
  const [rankingControls, setRankingControls] = useState({ metric: "", dimension: "", filterColumn: "", filterValue: "" });
  const [mixControls, setMixControls] = useState({ metric: "", dimension: "", filterColumn: "", filterValue: "" });
  const [mapControls, setMapControls] = useState({ metric: "", filterColumn: "", filterValue: "" });
  ...
  const visualRows = sheet?.rows || [];
  const metricByHeader = useMemo(() => new Map((sheet?.columns || []).filter((column) => column.isNumeric).map((column) => [column.header, column])), [sheet]);
  const dimensionByHeader = useMemo(() => new Map((sheet?.columns || []).filter((column) => column.isDimension).map((column) => [column.header, column])), [sheet]);
  const kpiMetric = metricByHeader.get(kpiControls.metric) || primaryMetric;
  const trendMetric = metricByHeader.get(trendControls.metric) || primaryMetric;
  const monthlyMetric = metricByHeader.get(monthlyControls.metric) || primaryMetric;
  const rankingMetric = metricByHeader.get(rankingControls.metric) || primaryMetric;
  const mixMetric = metricByHeader.get(mixControls.metric) || primaryMetric;
  const trendDimension = dimensionByHeader.get(trendControls.dimension) || primaryDimension;
  const monthlyDimension = dimensionByHeader.get(monthlyControls.dimension) || primaryDimension;
  ...
  const kpiRows = useMemo(() => applyVisualFilter(filteredRows, kpiControls), [filteredRows, kpiControls]);
  const trendRows = useMemo(() => applyVisualFilter(filteredRows, trendControls), [filteredRows, trendControls]);
  const monthlyRows = useMemo(() => applyVisualFilter(filteredRows, monthlyControls), [filteredRows, monthlyControls]);
  const rankingRows = useMemo(() => applyVisualFilter(filteredRows, rankingControls), [filteredRows, rankingControls]);
  const mixRows = useMemo(() => applyVisualFilter(filteredRows, mixControls), [filteredRows, mixControls]);
  ...
  function resetFilters() {
    ...
    setKpiControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
    setTrendControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
    setMonthlyControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
    setRankingControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
    setMixControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
    setMapControls((current) => ({ ...current, filterColumn: "", filterValue: "" }));
  }
