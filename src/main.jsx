...
function geometryBounds(features) {
  ...
  for (const feature of features) {
    for (const polygon of extractGeoParts(feature.geometry)) {
...
function ringPath(ring, bounds, width, height) {
  ...
  for (let i = 0; i < ring.length; i += 1) {
    const point = projectGeoPoint(ring[i], bounds, width, height);
    if (!Number.isFinite(point[0]) || !Number.isFinite(point[1])) continue;
    path += `${i === 0 ? "M" : "L"}${point[0].toFixed(2)},${point[1].toFixed(2)}`;
  }
  return `${path}Z`;
}

function featurePath(feature, bounds, width, height) {
  return extractGeoParts(feature.geometry).map((polygon) => polygon.map((ring) => ringPath(ring, bounds, width, height)).join(" ")).join(" ");
}
...
function MapCard({ rows, title = "Location map" }) {
  ...
  const colorForCount = (count) => {
    ...
    const hue = 215 - Math.round(ratio * 170);
    return `hsl(${hue} 78% ${58 - ratio * 12}%)`;
  };

  return <ChartCard wide title={title} subtitle={`Detected ${matchedStates.length} place${matchedStates.length === 1 ? "" : "s"} in the worksheet. Highlight intensity reflects occurrence count.`}>
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 190px", gap: 18, height: "100%", minHeight: 430 }}>
      ...
      <div style={{ borderLeft: "1px solid var(--border)", paddingLeft: 16, overflowY: "auto" }}>
        ...
        {[...stateCounts.entries()].sort((a, b) => b[1] - a[1]).map(([name, count]) => <div key={name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, padding: "7px 0", fontSize: 12 }}><span style={{ display: "flex", alignItems: "center", gap: 7 }}><span style={{ width: 10, height: 10, borderRadius: 3, background: colorForCount(count), border: "1px solid var(--border-strong)" }} />{name}</span><strong>{formatFullNumber(count)}</strong></div>)}
