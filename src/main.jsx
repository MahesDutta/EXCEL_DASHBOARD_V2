...
async function processWorkbookDirect(buffer, onProgress) {
  ...
  for (let index = 0; index < names.length; index += 1) {
    ...
    const start = 10 + Math.round((index / names.length) * 80);
    onProgress(start, `Analyzing sheet ${index + 1} of ${names.length}: ${name}`);
    await new Promise((resolve) => setTimeout(resolve, 0));
    const cleaned = cleanSheet(workbook.Sheets[name], name);
    if (cleaned?.rows?.length) sheets.push(cleaned);
    onProgress(10 + Math.round(((index + 1) / names.length) * 80), `Finished ${name}`);
    await new Promise((resolve) => setTimeout(resolve, 0));
  }

  if (!sheets.length) throw new Error("No usable table data was found in this workbook.");
  onProgress(96, "Finalizing dashboard data…");
  await new Promise((resolve) => setTimeout(resolve, 0));
  onProgress(100, "Workbook ready");
